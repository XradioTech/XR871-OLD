...
{
        if (BUG_ON(level > VOLUME_MAX_LEVEL || level < VOLUME_LEVEL0))
                return -1;

        int volume = level;
        AUDIO_Device dev = mc->current_outdev;

        if (BUG_ON(mc->playback == 0))
                return -1;
...
{
        if (BUG_ON(dev > AUDIO_DEVICE_SPEAKER || dev == 0))
                return -1;
...
{
        if (BUG_ON(dev != AUDIO_DEVICE_HEADPHONEMIC && dev != AUDIO_DEVICE_MAINMIC))
                return -1;
...
{
        if (BUG_ON(event >= AUDIO_DEVICE_MANAGER_NONE))
                return -1;
