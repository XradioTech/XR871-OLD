...
void mbedtls_server(void *arg)
{
        ...
        unsigned int milliseconds = param->continue_ms;
...
reset:
        ...
        if (flags & MBEDTLS_SSL_FLAG_SERVER)
                goto server;
        else if (flags & MBEDTLS_SSL_FLAG_WEBSERVER)
                goto webserver;
server:
                ...
                if( ret <= 0 )
                {
                        switch( ret )
                        {
                                ...
                                case MBEDTLS_ERR_NET_CONN_RESET:
                                        mbedtls_printf( " connection was reset by peer,goto RESET\n" );
                                        goto reset;
                                        break;
