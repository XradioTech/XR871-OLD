				       ...
				       const char      * origin)
{
	...
	if (enable_tls) {
		...
		if (options && options->ca_certificate) {
			if (mbedtls_x509_crt_parse(conn->ssl_cert, (const unsigned char *)options->ca_certificate, options->ca_certificate_size) != 0) {
		...
		mbedtls_ssl_conf_ca_chain(conn->ssl_conf, conn->ssl_cert->next, NULL);
