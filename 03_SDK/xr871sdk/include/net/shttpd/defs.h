...
#if defined(NO_FS)
		int   filelength;
	  	char		*filepointer;		/* Regular static file		*/
  	} fi;
	struct {
	  	int   filelength;
	  	unsigned int filepointer;		/* Regular static file		*/
  	} ex_fi;//file stored on flash
#endif
	struct {
...
struct io_class {
	...
	int (*io_read)(struct stream *, void *buf, size_t len);
	int (*io_write)(struct stream *, const void *buf, size_t len);
	void (*io_close)(struct stream *);
};
...
#define _shttpd_elog(flags, conn, fmt, arg...)	\
		do { \
			printf(fmt, ##arg); \
			printf("\n");\
			if (flags & E_FATAL) {\
...
#else
#define _shttpd_elog(flags, conn, fmt, arg...) NULL
#endif
#define _shttpd_log_access(fp, c) NULL
#else
...
#if defined(NO_FS)
extern char*		_shttpd_open(const char *path);
#else
